;!function (win) {
    ...
    Xadmin.prototype.init = function () {
        ...
        //处理刷新标签页记住问题
        var selectTabId = this.get_selectTab();
        element.tabChange('xbs_tab', selectTabId ? selectTabId.id : i);
    };
    /**
     * [end 执行结束要做的]
    ...
    Xadmin.prototype.end = function () {
        ...
        var selectTabId = this.get_selectTab();
        if (selectTabId) {
            this.switch_left_menu(selectTabId.id);
        } else {
            //处理默认的菜单展开
     ...
     * @param dId 删除的id
     */
    Xadmin.prototype.del_tab = function (dId) {
        if (dId) {
            console.log(88);
        } else {
     ...
     * @returns {boolean|*}
     */
    Xadmin.prototype.get_selectTab = function () {
        if (typeof is_remember != "undefined")
            return false;
        return layui.data('selectTab')
    }
    /**
     * [set_data 增加某一项]
    ...
    /**
     * [switch_left_menu 切换菜单到指定 id]
     * @param selectId 指定要选中的菜单
     */
    Xadmin.prototype.switch_left_menu = function (selectId) {
        //遍历所有的菜单 a-menu
        $('.a-menu').each(function () {
            var md5Url = md5($(this).data('url'));
            //判断 md5(url) 和 lay-id 相同的菜单
            if (md5Url === selectId) {
                //移除所有的 菜单 a标签的active class
                $('.left-nav').find('a').removeClass('active');
                $(this).addClass('active');//当前菜单添加active类
                var pLi = $(this).parent('li').parent().parent();//获取当前a标签的 父li元素的 父元素
                if (pLi.children('.sub-menu').length) {
                    //当前 伸缩菜单
                    pLi.addClass('open');
                    pLi.children('a').find('.nav_right').html('&#xe6a6;');
                    pLi.children('.sub-menu').stop(true, true).slideDown();
                    //其他伸缩菜单
                    pLi.siblings().children('.sub-menu').stop(true, true).slideUp();
                    pLi.siblings().find('.nav_right').html('&#xe697;');
                    pLi.siblings().removeClass('open');
                }
                return false;
            }
        });
...
layui.use(['layer', 'element', 'jquery'], function () {
    ...
    //记录当前切换的id，用于刷新页面时切换回原来的标签页 sxj add 2021年3月30日
    element.on('tab(xbs_tab)', function (data) {
        var id = $(this).attr('lay-id');
        layui.data('selectTab', {key: 'id', value: id});//保存
        //切换侧边菜单到指定的id
        xadmin.switch_left_menu(id);
    });
    ...
    $('.left-nav #nav').on('click', 'li', function (event) {
        ...
        // 判决 目录菜单有 href='javascript:;' 菜单没有href
        if (elA.attr('href') === undefined) {
            xadmin.add_tab(elA.data('title'), elA.data('url'));
        }
